...
export function isValid(question: Question, answer: string): boolean {
    ...
    // In case a new type is added in the future
    return false;
}
...
export function toMarkdown(question: Question): string {
     // Start with the first line (hash + name) and second line (body)
     let result = `# ${question.name}\n${question.body}`;

     // If multiple choice, add each option on a new line prefixed with "- "
...
export function mergeQuestion(
    ...
    return {
        ...
        options: [...contentQuestion.options], // make a copy to avoid mutation
